...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-3xl p-8 shadow-[0_2px_10px_rgba(0,0,0,0.04)] relative overflow-hidden flex flex-col justify-between h-72 border border-gray-50">
            <div class="w-2/3 z-10">
                <span id="dashBalanceBadge" class="inline-flex items-center bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide mb-4 transition-colors">
                    <div id="dashBalanceDot" class="w-1.5 h-1.5 bg-gray-400 rounded-full mr-1.5"></div> <span id="dashBalanceText">Menunggu Data</span>
                </span>
                <h3 id="dashMainTitle" class="text-3xl font-bold text-gray-900 leading-tight mb-3">Belum ada asupan yang tercatat hari ini.</h3>
                <p id="dashSubTitle" class="text-xs text-gray-500 leading-relaxed">Konfirmasi penerimaan makanan Anda untuk melihat analisis AI terhadap target nutrisi mikro harian Anda.</p>
            </div>
            ...
            <div class="flex space-x-4 mt-6 z-10">
                <div class="bg-[#F8F9FA] rounded-2xl px-5 py-3 border border-gray-100">
                    ...
                    <p class="text-lg font-bold text-gray-900"><span id="dashCalText" class="text-emerald-600">0</span><span id="dashTargetCalText" class="text-xs text-gray-400"> / 0</span></p>
                </div>
                <div class="bg-[#F8F9FA] rounded-2xl px-5 py-3 border border-gray-100">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Protein</p>
                    <p class="text-lg font-bold text-gray-900"><span id="dashProText" class="text-blue-600">0</span><span class="text-xs text-gray-400"> g</span></p>
                </div>
            </div>

            <div class="absolute right-4 top-1/2 -translate-y-1/2 w-48 h-48 opacity-90 flex items-center justify-center">
                <svg class="absolute top-0 right-8 w-16 h-16 text-gray-100" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z"/></svg>
                <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                    <circle cx="50" cy="50" r="40" fill="none" stroke="#F1F3F5" stroke-width="8"></circle>
                    <circle id="dashCircle" cx="50" cy="50" r="40" fill="none" stroke="#059669" stroke-width="8" stroke-dasharray="251.2" stroke-dashoffset="251.2" stroke-linecap="round" class="transition-all duration-1000 ease-out"></circle>
                </svg>
        ...
        <div id="notificationCard" class="hidden bg-white rounded-3xl p-6 shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-emerald-100 items-center justify-between transform transition-all duration-500 hover:scale-[1.01]">
            <div class="flex items-center space-x-4">
                <div class="w-12 h-12 rounded-full bg-green-50 flex items-center justify-center text-emerald-600 shrink-0">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path></svg>
                </div>
                <div>
                    <h4 class="font-bold text-gray-900 text-lg">Apakah kamu sudah menerima makanan bergizi hari ini?</h4>
                    <p class="text-sm text-gray-500 mt-1">Menu: <span id="foodListText" class="text-emerald-600 font-semibold">Memuat...</span></p>
                </div>
            </div>
            <div class="flex space-x-3 ml-4 shrink-0">
                <button onclick="submitAnswer('Ya')" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 px-6 rounded-xl flex items-center text-sm shadow-md transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Ya
                </button>
                <button onclick="submitAnswer('Tidak')" class="bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-2.5 px-6 rounded-xl flex items-center text-sm transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg> Tidak
                </button>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-gray-50 pb-8">
            ...
            <table class="w-full text-left">
                ...
                <tbody id="historyTableBody" class="text-sm">
                    <tr><td colspan="3" class="py-8 text-center text-gray-400">Memuat riwayat...</td></tr>
                </tbody>
            </table>
        </div>
    </div>
    ...
    <div class="lg:col-span-1">
        <div class="bg-blue-50/50 rounded-3xl p-6 border border-blue-100/50 h-full">
            <h3 class="font-bold text-lg text-gray-900 flex items-center mb-6">
                <svg class="w-5 h-5 mr-2 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                Peringatan<br>Kesehatan
            </h3>
            
            <div class="space-y-4" id="healthWarningContainer">
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 text-center text-sm text-gray-400">
                    Menghitung profil gizi Anda...
                </div>
            </div>
            
            <div class="mt-6 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-4 border border-emerald-100/50 text-center">
                <p class="text-xs italic text-emerald-800 font-medium">"Kesehatan adalah kekayaan terbesar." — GIZENIA</p>
            </div>
        </div>
    </div>
</div>

<div id="nutritionCheckModal" class="fixed inset-0 z-[80] hidden bg-gray-900/70 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all p-6">
        <div class="text-center mb-6">
            <div id="modalIconBox" class="w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-4">
        ...
        <div id="balanceStatus" class="flex items-center p-4 bg-emerald-50 text-emerald-700 rounded-xl text-sm font-semibold mb-6 border border-emerald-100 leading-snug">
            </div>

        <button onclick="closeNutritionModal()" class="w-full bg-gray-900 hover:bg-black text-white font-bold py-3.5 px-4 rounded-xl transition-colors shadow-lg">
