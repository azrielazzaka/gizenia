...
<head>
    ...
    <title>GIZENIA AI - Admin Dashboard</title>
...
<body class="bg-[#F8F9FA] text-gray-800 flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-[#F4F6F8] border-r border-gray-200 h-full flex flex-col justify-between hidden md:flex">
        ...
        <div class="px-4 py-6 space-y-4">
            <a href="{{ route('login') }}"
                class="flex items-center px-4 py-2 text-sm text-gray-500 hover:text-red-600 transition-colors"
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden">
        <header class="h-20 flex items-center justify-between px-8 bg-[#F8F9FA]">
            <div class="relative w-96" id="searchContainer">
                ...
                <input type="text" id="globalSearchInput" onfocus="showSearchDropdown()"
                    oninput="filterSearch(this.value)" placeholder="Cari halaman, fitur, menu..."
                    class="w-full bg-[#Eef0f2] border-none rounded-full pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-inner transition-all">

                <div id="searchDropdown"
                    class="hidden absolute left-0 top-full mt-2 w-full bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50 transform opacity-100 transition-all duration-200">
                    <div
                        class="px-4 py-2.5 bg-gray-50/80 border-b border-gray-50 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                        Rekomendasi Halaman
                    </div>
            ...
            <div class="flex items-center space-x-5 relative">
                ...
                <!-- 🔽 DROPDOWN SETTING -->
                <div id="settingMenu" class="hidden absolute right-full mr-3 top-12 w-48
