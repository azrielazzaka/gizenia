...
<script>
    ...
    async function loadMenusForDropdown() {
        try {
            ...
            availableMenus = await res.json();
        } catch (e) { console.error("Gagal memuat menu"); }
    }

    function populateSelect(selectElement) {
        selectElement.innerHTML = '<option value="">-- Pilih Menu Makanan --</option>';
        availableMenus.forEach(menu => {
            const id = menu._id || menu.id;
            selectElement.innerHTML += `<option value="${id}" data-name="${menu.name}">${menu.name}</option>`;
        });
    }

    // Fungsi Tambah Baris Makanan
    function addFoodRow(menuId = '', weight = '') {
        ...
        row.innerHTML = `
            ...
            <input type="number" class="food-weight w-32 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none" placeholder="Berat (g)" value="${weight}" required>
            <button type="button" onclick="this.parentElement.remove()" class="text-red-400 hover:text-red-600 px-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
        `;
        container.appendChild(row);
        const select = row.querySelector('.food-select');
        populateSelect(select);
        if(menuId) select.value = menuId;
    }
    ...
    async function fetchDistributions(page = 1) {
        currentPage = page;
        try {
            const res = await fetch(`/api/distributions?page=${page}`, { headers: { 'Authorization': `Bearer ${token}` }});
            const data = await res.json();
            allDistData = data.data; // Simpan untuk Edit
            
            const tbody = document.getElementById('distTableBody');
            tbody.innerHTML = '';

            if (data.data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" class="py-8 text-center text-gray-400">Belum ada riwayat distribusi.</td></tr>';
            } else {
                data.data.forEach(dist => {
                    const distId = dist._id || dist.id;
                    const foodHtml = dist.foods.map(f => `<span class="inline-block bg-emerald-50 text-emerald-700 text-[10px] font-bold px-2 py-1 rounded mr-1 mb-1">${f.name} (${f.weight}g)</span>`).join('');
                    const classHtml = dist.target_classes.map(c => `<span class="inline-block bg-blue-50 text-blue-600 text-[10px] font-bold px-2 py-1 rounded mr-1 mb-1">${c}</span>`).join('');
                    
                    tbody.innerHTML += `
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="py-4 px-6 font-bold text-gray-800 text-xs">${dist.distribution_date}</td>
                            <td class="py-4 px-6">${foodHtml}</td>
                            <td class="py-4 px-6">${classHtml}</td>
                            <td class="py-4 px-6 text-center"><span class="text-xs font-bold text-emerald-600"><i class="fas fa-check-circle"></i> Terkirim</span></td>
                            <td class="py-4 px-6 text-right space-x-3">
                                <button onclick="editDist('${distId}')" class="text-blue-500 hover:text-blue-700 font-semibold text-xs">Edit</button>
                                <button onclick="promptDeleteDist('${distId}')" class="text-red-500 hover:text-red-700 font-semibold text-xs">Hapus</button>
                            </td>
                        </tr>
                    `;
                });
            ...
            // Render Paginasi
            document.getElementById('paginationInfo').innerText = `Menampilkan ${data.from || 0} - ${data.to || 0} dari ${data.total} distribusi`;
            const controls = document.getElementById('paginationControls');
            controls.innerHTML = '';
            if (data.current_page > 1) controls.innerHTML += `<button onclick="fetchDistributions(${data.current_page - 1})" class="px-3 py-1 bg-white border border-gray-200 text-gray-600 rounded-md hover:bg-gray-50 text-xs font-medium transition">Prev</button>`;
            controls.innerHTML += `<span class="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-md text-xs font-bold">${data.current_page} / ${data.last_page}</span>`;
            if (data.current_page < data.last_page) controls.innerHTML += `<button onclick="fetchDistributions(${data.current_page + 1})" class="px-3 py-1 bg-white border border-gray-200 text-gray-600 rounded-md hover:bg-gray-50 text-xs font-medium transition">Next</button>`;

        } catch (e) { console.error(e); }
    }

    // 3. CREATE / EDIT Modal Logic
    function openModalForAdd() {
        isEditMode = false;
        currentDistId = null;
        document.getElementById('distForm').reset();
        document.getElementById('modalTitle').innerText = "Distribusi Makanan Baru";
        document.getElementById('foodListContainer').innerHTML = '';
        addFoodRow(); // Tambah 1 baris kosong
        
        // Uncheck all
        document.querySelectorAll('.class-checkbox').forEach(cb => cb.checked = false);
        toggleModal('distModal');
    }

    function editDist(id) {
        isEditMode = true;
        currentDistId = id;
        document.getElementById('modalTitle').innerText = "Edit Distribusi Makanan";
        
        const dist = allDistData.find(d => (d._id === id || d.id === id));
        if(dist) {
            // Isi ulang makanan
            document.getElementById('foodListContainer').innerHTML = '';
            dist.foods.forEach(f => addFoodRow(f.menu_id, f.weight));
            
            // Isi ulang checkbox kelas
            document.querySelectorAll('.class-checkbox').forEach(cb => cb.checked = false);
            dist.target_classes.forEach(c => {
                const cb = document.querySelector(`.class-checkbox[value="${c}"]`);
    ...
    // 4. Submit -> Buka Modal Konfirmasi
    let payloadToSubmit = null; // Simpan sementara data sebelum dieksekusi
    ...
    document.getElementById('distForm').addEventListener('submit', function(e) {
        ...
        const targetClasses = [];
        document.querySelectorAll('.class-checkbox:checked').forEach(cb => targetClasses.push(cb.value));

        if(foods.length === 0) return alert('Pilih minimal 1 makanan!');
        if(targetClasses.length === 0) return alert('Pilih minimal 1 kelas penerima!');

        payloadToSubmit = { foods, target_classes: targetClasses };

        // Buka Pop-up Konfirmasi
        toggleModal('distModal'); // Tutup modal form
        toggleModal('confirmDistModal'); // Buka modal konfirmasi
    });

    // 5. Eksekusi API Distribusi (Setelah Klik Konfirmasi)
    async function executeDistribute() {
        const btn = document.getElementById('btnExecuteDist');
        btn.innerText = 'Menyebarkan...'; btn.disabled = true;

        const url = isEditMode ? `/api/distributions/${currentDistId}` : '/api/distributions';
        ...
        try {
            ...
            if(res.ok) {
                toggleModal('confirmDistModal'); // Tutup modal biru
                fetchDistributions(currentPage); // Refresh tabel
                
                // MUNCULKAN POP-UP SUKSES
                setTimeout(() => toggleModal('successDistModal'), 300);
            }
        } catch(e) { alert('Error server'); }
        finally { btn.innerText = 'Ya, Sebarkan!'; btn.disabled = false; }
    }

    // 6. DELETE Logic
    function promptDeleteDist(id) {
        distToDeleteId = id;
        toggleModal('deleteDistModal');
    }

    async function executeDeleteDist() {
        try {
            const res = await fetch(`/api/distributions/${distToDeleteId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` }});
            if (res.ok) {
                toggleModal('deleteDistModal');
    ...
    // Inisialisasi awal
    loadMenusForDropdown().then(() => fetchDistributions(1));
</script>
@endsection